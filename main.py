    ...
    if conn:
        try:
            with conn.cursor() as cur:
                ...
                """)
                conn.commit()
            print("Tabela api_keys verificada/criada.")
        except Exception as e:
...
# ==========================================
app = FastAPI(title="Synapse-LangID Enterprise Auth", version="4.1.2")
...
def revogar_chave(request: Request, key: str = Depends(verify_api_key)):
    ...
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
        return {"message": "Sua chave foi revogada com sucesso. Seu IP está liberado para gerar uma nova."}
    finally:
        conn.close()
