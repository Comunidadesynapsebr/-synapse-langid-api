...
def get_db_connection():
    ...
    if not db_url:
        print("Aviso: DATABASE_URL não configurada no ambiente.")
        return None
    
    # Remove quebras de linha, retornos de carro e espaços nas pontas
...
# ==========================================
app = FastAPI(title="Synapse-LangID Enterprise Auth", version="4.1.1")
...
def info_chave(request: Request, key: str = Depends(verify_api_key)):
    """Retorna os dados da chave de API atual."""
    if key == MASTER_KEY:
...
def listar_idiomas(request: Request):
    """Lista de idiomas suportados pelo modelo (Endpoint Público)."""
    return {
        "model": MODEL_ID,
        "supported_languages_count": 20,
        "examples": ["pt (Português)", "en (Inglês)", "es (Espanhol)", "fr (Francês)", "de (Alemão)"],
        "note": "Detecção automática de idiomas baseada no dataset de treinamento."
    }
                    ip TEXT PRIMARY KEY,
                    key TEXT UNIQUE NOT NULL,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
